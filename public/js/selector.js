  ...
  $("select option:selected").each(function () {
    $.post("/scheduler/order/" + this.value, {
      ...
      $.post("/scheduler/wagon/" + starting_station_id, {
        ...
        for (var _i = 0; _i < wagons.length; _i++) {
          if (wagons[_i] == 1) {
...
window.processShipment = function () {
  ...
  function shipment(wagons_needed, order_id, starting_station_id, destination_station_id, current_shippping_date, deadline) {
    ...
    $.post("/scheduler/create_train/" + wagons_needed, {
      ...
      for (var i = 0; i < wagons_needed.length; i++) {
        $.post("/scheduler/wagon_paired/" + train_id, {
          '_token': $('meta[name=csrf-token]').attr('content'),
          'wagon_id': wagons_needed[i]
        }).done(function (data) {});
      }
  ...
  var starting_station_id = $("#starting_station_id").text().split(':')[1]; //console.log(starting_station_id);
  ...
  $.post("/scheduler/wagon/" + starting_station_id, {
    ...
    if (orders_paired_to_station_flag == 0) {
      for (var x = 0; x < shippin_dates.length; x++) {
        ...
        var station_capacity_per_type = []; //sum wagon capacity using type id as the index
...
window.refreshOpening = function () {
  ...
  var current_row_pos = change[1];
  var current_cell_pos = change[2];
  var source_row_pos = change[4];
  var source_cell_pos = change[5]; //let columns_to_refresh=change[]

  var row_ids = [];
  $("#table1 tr").each(function () {
    row_ids.push($(this).attr('id'));
  });
  var cells = $("#" + row_ids[current_row_pos] + " td");
  var source_cells = $("#" + row_ids[source_row_pos] + " td");
  var source_cell = source_cells[source_cell_pos - 1];
  var actual_cell = cells[current_cell_pos - 1]; //console.log(actual_cell);

  var actual_cell_id = actual_cell.id; //console.log(source_cell);

  var source_cell_id = source_cell.id;

  if (actual_cell_id.split('_')[0] == "station") {
    var actual_type = $("#" + actual_cell_id + " div").attr('id').split('_')[0];
    var capacity_rows = [];
    $("#capacity_" + actual_type + " td").each(function () {
      capacity_rows.push($(this));
    });
    var subtract = capacity_rows[current_cell_pos - 1]; //subtract.first();

    subtract = subtract.attr('id'); //console.log(subtract);
    ...
    if (source_cell_id.split('_')[0] == "station") {
      var adding = capacity_rows[source_cell_pos - 1];
...
window.onload = function () {
  ...
  rd.init(); // define  class name as exception for cells
  ...
  console.log("proba"); // event handler called after DIV element is dropped to TD

  rd.event.dropped = function (targetCell) {
    console.log("proba");
    var divClass = rd.mark.exceptionClass; // DIV exception class
    // if the DIV element was dropped to allowed cell

    if (targetCell.className.indexOf(divClass.type1) > -1 || targetCell.className.indexOf(divClass.type2) > -1 || targetCell.className.indexOf(divClass.type3) > -1 || targetCell.className.indexOf(divClass.type4) > -1) {
      // make DIV unmovable
...
module.exports = __webpack_require__(/*! C:\Users\József\Desktop\Szakdolgozat\resources\js\selector.js */"./resources/js/selector.js");
