...
window.processShipment = function () {
  ...
  function shipment(wagons_needed, order_id, starting_station_id, destination_station_id, current_shippping_date, deadline) {
    var time_span = datediff(parseDate(current_shippping_date), parseDate(deadline)); //console.log(time_span);
    ...
    $.post("/scheduler/create_train/" + wagons_needed, {
      ...
      $.post("/scheduler/get_route/", {
        ...
        var route_id = data; //console.log(order_id,train_id,route_id,time_span,current_shippping_date);
  ...
  $.post("/scheduler/wagon/" + starting_station_id, {
    ...
    if (orders_paired_to_station_flag == 0) {
      for (var x = 0; x < shippin_dates.length; x++) {
        ...
        var station_capacity_per_type = []; //console.log(wagons.length);
        ...
        for (var i = 0; i < ordered_types.length; i++) {
          if (station_capacity_per_type[ordered_types[i]] < ordered_quantities[i]) {
            capacity_flag = 1; //console.log("types:"+ordered_types[i]);
